...
def run_experiment(approximator,queue):
    ...
    queue.put((env.score,env.board))  # Store the score in the queue
...
if __name__ == "__main__":
    num_processes = 10
    processes = []
    mp.set_start_method("fork")  # Important: Fixes multiprocessing issues on macOS
    ...
    min_score = float('inf')
    max_score = float('-inf')
    for score,board in result:
        scores += score
        max_score = max(max_score,score)
        min_score = min(min_score,score)
        print(board,score,sep='\n')
